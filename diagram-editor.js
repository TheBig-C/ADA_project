...
DiagramEditor.prototype.save = function(data, draft, elt) {
	// Llama a la función `done` para realizar el guardado
	//this.done(data, draft, elt);
  console.log('data: ',data)



	// Utilizar DOMParser para convertir la cadena de datos XML en un documento XML
	var parser = new DOMParser();
	var xmlDoc = parser.parseFromString(data, 'application/xml');
  
	// Acceder a los elementos <mxCell>
	var celdas = xmlDoc.getElementsByTagName('mxCell');
  
	// Iterar sobre las celdas y acceder a sus valores y valores de flecha
	for (var i = 0; i < celdas.length; i++) {
	  // Acceder al atributo 'value' de cada celda
	  var valorCelda = celdas[i].getAttribute('value');
  
	  // Acceder al elemento hijo 'mxGeometry' de cada celda
	  var geometryElement = celdas[i].getElementsByTagName('mxGeometry')[0];
  
	  // Acceder al valor dentro del elemento 'mxGeometry' (asumiendo que contiene el valor de la flecha)
	  //var valorFlecha = geometryElement.textContent.trim();
  
	  console.log('Valor de la celda:', valorCelda);
	  console.log('Valor de la flecha:');
	}
  };
